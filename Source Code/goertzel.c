...
float goertzel_algorithm(float signals[], int number_of_samples, float wanted_frequency, float sampleRate, float* out_magnitude)
{
...
void tone_to_bits(float* signal, int signal_length, char* bit_stream, int* num_bits)
{
    ...
    *num_bits = 0;
    ...
    for (int i = 0; i < total_frames; i++)
    {
        ...
        float magnitude;
        float power = goertzel_algorithm(frame_start, FRAME_SIZE, TARGET_FREQ, SAMPLE_RATE, &magnitude);
